...
class Clock(object):
    ...
    def draw_clock_face(self):
        ...
        # after.
        utc_hour = (value_return.get_hour(self.now)) % 24

        self.hour = value_return.time_from_utc(self.diff, utc_hour)
    ...
    def draw_minute_hand(self):
        self.minutes = value_return.get_minutes((self.now))
        minutes_angle = self.minutes / 60 * 360

        if value_return.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif value_return.time_from_utc(0, self.diff) % 1 == 0.75:
           minutes_angle += 270
    ...
    def draw_second_hand(self):
        self.seconds = value_return.get_seconds(self.now)
    ...
    def draw_time_text(self):
        self.canvas.create_text(95, 225, text=value_return.get_time(int(self.hour), int(self.minutes),
